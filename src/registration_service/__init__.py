        ...
        if res_app_obj.check_db_connectivity_and_retry():
            if res_app_obj.init_databases_for_service():
                if res_app_obj.create_tables_associated_to_db_model():
                    ...
                    registration_app_logger.info("Registered GRPC server to the server :: {0}".format("UserValidationForTokenGenerationService"))
                    server.add_insecure_port(registration_app.config["REGISTRATION_GRPC_SERVER_IP"]+":"+registration_app.config["REGISTRATION_GRPC_SERVER_PORT"])
                    registration_app_logger.info("Starting GRPC server for the Token-User service with the IP & PORT:: {0}:{1}".format(registration_app.config["REGISTRATION_GRPC_SERVER_IP"], registration_app.config["REGISTRATION_GRPC_SERVER_PORT"]))
