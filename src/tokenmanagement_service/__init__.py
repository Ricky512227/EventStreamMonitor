...
try:
    ...
    if loaded:
        ...
        getuser_headers_schema_status, getuser_headers_schema = usermanager.read_json_schema(getuser_headers_schema_filepath)
        del_user_headers_schema_status, del_user_headers_schema = usermanager.read_json_schema(del_user_headers_schema_filepath)
            ...
            if usermanager_db_obj.check_db_connectivity_and_retry(db_engine=user_management_db_engine):
                ...
                if usermanager_db_obj.create_database_for_service():
                    ...
                    if usermanager_db_obj.create_tables_associated_to_db_model(db_engine=user_management_db_engine):
                        ...
                        user_management_migrate = usermanager_db_obj.migrate_db_bind_app(app_instance=usermanager_app, app_db=user_management_SQLAlchemy)
                        # Initialize/Create a pool of connections for the service
                        user_management_connection_pool = usermanager_db_obj.create_pool_of_connections(db_engine=user_management_db_engine)

                        user_management_session_maker = usermanager_db_obj.create_session_maker_to_connectio_pool(db_engine= user_management_db_engine, connection_pool=user_management_connection_pool)
                        ...
                        usermanager.register_blueprint_for_service(app_instance=usermanager_app, blueprint_instance=usermanager_bp)
