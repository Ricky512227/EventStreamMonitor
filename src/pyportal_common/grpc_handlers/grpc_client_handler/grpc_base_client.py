...
class PyportalGrpcBaseClient:
    ...
    def create_my_channel(self):
        ...
        self.base_grpc_channel = grpc.insecure_channel(
            self.grpc_base_client_ip + ":" + self.grpc_base_client_port)
    ...
    def assign_stub_to_channel(self, stub_cls_name):
        try:
            if self.base_grpc_channel:
                print("Assigning Stub and to the channel :: {0}".format(
                    self.base_grpc_channel))
                self.base_stub = stub_cls_name(self.base_grpc_channel)
                print(
                    "Created Stub is :: {0} and assigned to the channel :: {1}"
                    .format(self.base_stub, self.base_grpc_channel))
                return self.base_stub
        except Exception as ex:
            print("Error occurred :: {0}\tLine No:: {1}".format(
                ex,
