...
class AppHandler:
    ...
    def create_app_instance(self) -> Union[Flask, None]:
        ...
        Returns:
            Union[Flask, None]: The Flask application instance or None if an exception occurs.
        """
        try:
            self.cmn_logger.info(f"Received Service [{self.cmn_logger.name}]")
            app_instance = Flask(self.cmn_logger.name)
            self.cmn_logger.info(
                f"Created Flask Application Instance for service [{self.cmn_logger.name}]"
            )
            return app_instance
    ...
    def bind_jwt_manger_to_app_instance(
        ...
        Returns:
            Union[JWTManager, None]: The JWTManager instance or None if an exception occurs.
        """
        try:
    ...
    def create_blueprint_instance(self) -> Union[Blueprint, None]:
        ...
        Returns:
            Union[Blueprint, None]: The Blueprint instance or None if an exception occurs.
        """
        try:
            blueprint_instance = Blueprint(self.cmn_logger.name + "_bp",
                                           __name__)
            self.cmn_logger.info(
                f"Created Blueprint :: {blueprint_instance} for the Application service ::{self.cmn_logger.name}"
            )
    ...
    def read_json_schema(
        ...
        Returns:
            tuple[bool, Union[dict[Any, Any], dict[str, Any], dict[str, str]]]: A tuple
            containing a boolean indicating success or failure of schema loading and the loaded schema.
        """
        try:
            self.cmn_logger.info(
    ...
    def generate_req_missing_params(self, rec_req_params, loaded_schema):
        """
        Generate a dictionary of missing required parameters in a request based on a JSON schema.
        ...
        Returns:
            dict: A dictionary containing missing required parameters, if any.
        """
        try:
            ...
            for key, value in rec_req_params.items():
                self.cmn_logger.info(
                    f"REQUEST <==> params ==> Param :: {key} :: Value :: {value} :: Type ::{type(value)} "
                )

            try:
                jsonschema.validate(instance=rec_req_params,
                ...
                self.cmn_logger.info(
                    f"Required_Properties_As_Per_Schema :: {required_properties}"
                )
                ...
                self.cmn_logger.info(
                    f"Packing message for missing property which are found :: {missing_params}"
                )
