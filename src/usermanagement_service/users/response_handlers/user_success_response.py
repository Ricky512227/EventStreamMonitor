...
class SuccessBaseResponseBody(ABC):
    def __init__(self, response_body):
        ...
        user_management_logger.info(
            "Initialised response_body: {0}".format(self.response_body)
        )
    ...
    def prepare_success_response_body(self):
        ...
        user_management_logger.info(
            "Generating Success response  :: [STARTED] :: {0}".format(user_instance)
        )
        success_response_body = {
            "user": {
                ...
                "CreatedAt": user_instance["data"]["CreatedAt"].strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
                "UpdatedAt": user_instance["data"]["UpdatedAt"].strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            }
        }
        user_management_logger.info(
            "Generating Success response  :: [SUCCESS] :: {0}".format(
                success_response_body
            )
        )
    ...
    def serialize_prepared_success_response_body(self):
        ...
        user_management_logger.info(
            "Serializing the response data :: [STARTED] :: {0} :: {1}".format(
                prepare_success_response_body, type(prepare_success_response_body)
            )
        )
        user_serialized_response_data = json.dumps(prepare_success_response_body)
        user_management_logger.info(
            "Serialized the response data  :: [SUCCESS] :: {0} :: {1}".format(
                user_serialized_response_data, type(user_serialized_response_data)
            )
        )
