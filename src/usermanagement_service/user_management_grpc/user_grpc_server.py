...
class UserValidationForTokenGenerationService(token_pb2_grpc.UserValidationForTokenGenerationServiceServicer):
    def ValidateUserCredentials(self, request, context):
        try:
            ...
            while True:
                ...
                response = token_pb2.TokenResponseMessage(result=f"Progress: {elapsed_time:.2f} seconds")
            ...
            if context.deadline:
                ...
                for key, value in vars(token_res_message).items():
                    user_management_logger.info("Received token_res_message-{0}: {1}".format(key, value))
                try:
                    user_data, is_exists = is_userid_exists(session_instance="xtyyy", userid=request.user_id)
                    if is_exists:
