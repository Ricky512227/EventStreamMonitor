...
from src.usermanagement_service import user_management_logger, req_headers_schema, reg_user_req_schema, user_management_session_maker, usermanager_db_obj, usermanager
from src.pyportal_common.error_handlers.invalid_request_handler import send_invalid_request_error_to_client
from src.pyportal_common.error_handlers.internal_server_error_handler import send_internal_server_error_to_client
from src.usermanagement_service.utils.util_helpers import is_username_email_already_exists_in_db, convert_db_model_to_resp
from src.usermanagement_service.users.response_handlers.create_user_success_response import CreateSuccessResponse


def register_user():
    try:
        user_management_logger.info('REQUEST ==> Received Endpoint for the request:: {0}'.format(request.endpoint))
        user_management_logger.info('REQUEST ==> Received url for the request :: {0}'.format(request.url))
        if request.method == 'POST':
            rec_req_headers = dict(request.headers)
            user_management_logger.info("Received Headers from the request :: {0}".format(rec_req_headers))
            ''' 
            ...
            if len(body_result.keys()) != 0:
                return send_invalid_request_error_to_client(app_logger_name=user_management_logger, message_data="Request Params Missing", err_details=body_result)
            ...
            session_to_validate_existing_user = usermanager_db_obj.get_session_from_conn_pool(session_maker_to_get_session=user_management_session_maker)
            ...
            if is_username_email_already_exists_in_db(session_instance=session_to_validate_existing_user, uname=username, email=emailaddress):
                usermanager_db_obj.close_session(session_instance=session_to_validate_existing_user)
                return send_invalid_request_error_to_client(app_logger_name=user_management_logger, message_data="Username or EmailAddress already exists")
            # Register user-logic begins here
            session_to_create_new_user = usermanager_db_obj.get_session_from_conn_pool(session_maker_to_get_session=user_management_session_maker)
            ...
            if session_to_create_new_user.is_active:
                try:
                    ...
                    if user_obj is None:
                        usermanager_db_obj.close_session(sesionname=session_to_create_new_user)
                        return send_internal_server_error_to_client(app_logger_name=user_management_logger, message_data="Create User Failed")
                    '''
                    ...
                    if len(user_instance.keys()) <= 0:
                        usermanager_db_obj.close_session(session_instance=session_to_create_new_user)
                        return send_internal_server_error_to_client(app_logger_name=user_management_logger, message_data="User Instance creation Failed")
                    '''
                    ...
                    with session_to_create_new_user.begin():
                        user_map_db_instance = user_obj.map_user_instance_to_db_model()
                        ...
                        '''
                        user_management_logger.info("Data adding into  DataBase session {0}:: [STARTED]".format(user_map_db_instance))
                        session_to_create_new_user.add(user_map_db_instance)
                        user_management_logger.info("Data added into  DataBase session {0}:: [SUCCESS]".format(user_map_db_instance))
                        user_management_logger.info("Added Data is committed into DataBase of session {0}:: [SUCCESS]".format(session_to_create_new_user))
                    ...
                    usermanager_db_obj.close_session(session_instance=session_to_create_new_user)
                    print("Error occurred :: {0}\tLine No:: {1}".format(ex, sys.exc_info()[2].tb_lineno))
                    return send_internal_server_error_to_client(app_logger_name=user_management_logger, message_data="Internal Server Error")
                else:
                    ''' 
                    ...
                    '''
                    user_instance = convert_db_model_to_resp(model_instance=user_map_db_instance)
                    if len(user_instance.keys()) <= 0:
                        usermanager_db_obj.close_session(session_instance=session_to_create_new_user)
                        return send_internal_server_error_to_client(app_logger_name=user_management_logger, message_data="User Response creation Failed")

                    suc_res_obj = CreateSuccessResponse(response_body=user_instance)
                    custom_user_response_body = suc_res_obj.generate_success_response()

                    if len(custom_user_response_body) == 0:
                        usermanager_db_obj.close_session(session_instance=session_to_create_new_user)
                        return send_internal_server_error_to_client(app_logger_name=user_management_logger, message_data="User success Response creation Failed")
                    ...
                    user_management_logger.info("Prepared success response and sending back to client  {0}:: [STARTED]".format(reg_usr_response))
                    usermanager_db_obj.close_session(session_instance=session_to_create_new_user)
        ...
        print("Error occurred :: {0}\tLine No:: {1}".format(ex, sys.exc_info()[2].tb_lineno))
        return send_internal_server_error_to_client(app_logger_name=user_management_logger, message_data="Unknown error caused")
