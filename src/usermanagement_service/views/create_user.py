...
from src.pyportal_common.db_handlers.db_utilities import get_session_from_pool, close_session
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if is_username_email_already_exists_in_db(
                ...
                return send_invalid_request_error_to_client(
                    app_logger_name=user_management_logger,
                    message_data="Username or EmailAddress already exists",
                )
            ...
            if session_to_create_new_user.is_active:
                try:
                    ...
                    if user_obj is None:
                        close_session(
                            sesionname=session_to_create_new_user
                        )
                        return send_internal_server_error_to_client(
                            app_logger_name=user_management_logger,
                    ...
                    return send_internal_server_error_to_client(
                        app_logger_name=user_management_logger,
                        message_data="Internal Server Error",
                    )
                else:
                    """
                    ...
                    """
                    user_instance = convert_db_model_to_resp(
                        model_instance=user_db_record_to_insert
                    )
                    if len(user_instance.keys()) <= 0:
                        close_session(
                            session_instance=session_to_create_new_user
