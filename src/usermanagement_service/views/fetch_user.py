...
from pyportal_common.error_handlers.base_error_handler import PyPortalAdminInvalidRequestError, PyPortalAdminInternalServerError, \
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            if get_user_management_session is None:
                session_err_res = PyPortalAdminInternalServerError(message="Create Session Failed", logger=user_management_app_logger)
            ...
            if get_user_management_session.is_active:
                try:
                    ...
                    if len(get_user_instance.keys()) <= 0:
                        invalid_req_err_res = PyPortalAdminInternalServerError(message="Get User Response creation Failed", logger=user_management_app_logger)
                        usermanager.close_session(sessionname=get_user_management_session)
                    ...
                    if len(custom_user_response_body.keys()) <= 0:
                        invalid_req_err_res = PyPortalAdminInternalServerError(message="Get User success Response creation Failed", logger=user_management_app_logger)
                        usermanager.close_session(sessionname=get_user_management_session)
                        return invalid_req_err_res.send_response_to_client()
